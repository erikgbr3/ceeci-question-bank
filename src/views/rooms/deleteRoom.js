...
const DeleteRoomView = ( { room, isVisible, closeModal, handleRoomDelete } ) => {

  const deleteRoom = async () => {
    try {
      const deleteRoomData = await RoomController.deleteOneRoom(room.id);
    ...
    <View style={styles.modalContainer}>
      <View style={styles.modalContent}>
        <View style={{ borderBottomColor: 'gray', borderBottomWidth: 1, width: '100%', padding: 5 }}>
          <Text style={{ fontSize: 20, textAlign: 'center' }}>¿Quieres Eliminar a {room.name}?</Text>
        </View>
        <View>
          <Button style={styles.button} buttonColor='#f45572' onPress={deleteRoom}>
